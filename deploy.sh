...
startup() {
  ...
  if [ $? -ne 0 ]; then
    helm upgrade --install $INFRA_NAME test-hive44-infrastructure \
      --values test-hive44-infrastructure/values.yaml \
      --values test-hive44-infrastructure/values-dev.yaml
...
deploy() {
  ...
  if is_app_running; then
    echo "test-hive44 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying test-hive44 from branch '$branch'..."
...
down() {
  if is_app_running; then
    ...
    echo "test-hive44 is not deployed"
  fi
}
